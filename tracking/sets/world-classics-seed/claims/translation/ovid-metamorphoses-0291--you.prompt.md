...
3. Save JSON output under tracking/sets/world-classics-seed/outputs/translation/<chunk-id>.json.
4. Include source URL, source refs, model/command route, uncertainties, and review notes.

For TRANSLATION stage:
- Fetch the public source from each chunk sourceLocator.
- Snap the planned 0.1% span to natural boundaries without overlap.
- Produce source refs, source excerpt, literal translation, reader translation, detailed glosses, uncertainties, review notes, and illustrationPromptSeed.
- Use high reasoning effort when available.

Use account-auth CLI routes only; do not use API-key SDK calls.
Suggested command:
grok --prompt-file <this-file> --output-format json --effort max